...
object Score {
    ...
    fun scoreFrame(currentFrame: Frame, nextFrame: Frame?, nextOfTheFollowing: Frame?): Int {
        val (firstNextRoll, secondNextRoll) = getNextTwoBalls(nextFrame, nextOfTheFollowing)
    ...
    private fun getNextTwoBalls(nextFrame: Frame?, nextOfTheFollowing: Frame?): Pair<Int, Int> =
            when (nextFrame) {
                ...
                is Tenth? -> getNextTwoBalls(nextFrame?.frame, OpenFrame(nextFrame?.firstExtraBall ?: ZERO, ZERO))
                null -> Pair(ZERO, ZERO)
            }
}
